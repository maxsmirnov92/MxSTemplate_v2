...
class CodeTasks {
    ...
    @Test
    fun lengthOfLongestUniqueSubstringTest() {
        val result = lengthOfLongestUniqueSubstring("asjrgapa")
...
//            previousChar = currentChar
    ...
    fun findMinPath(matrix: Array<IntArray>): PathResult {
        ...
        println("Found min path sum ${result.sum} in matrix with size $size in ${result.iterationsCount} iterations, " +
    ...
    fun findMaxPath(matrix: Array<IntArray>): PathResult {
        ...
        val size = matrix.sumOf { it.size }
        println("Found max path sum ${result.sum} in matrix with size $size in ${result.iterationsCount} iterations, " +
...
//                                            size - count,
                                            true
                                        ).also { count += it.iterationsCount }.sum.compare(
                                            findPathInternal(
                                                matrix,
                                                isMin,
                                                rowIndex + 1,
...
//                                            size - count,
                                                true
                                            ).also { count + it.iterationsCount }.sum
                                        ) // nextInColumn
                                    } else {

                ...
                if (!it.isLoading) {
                    ...
                    if (pendingStartService.get()) {
                        if (baseApplicationContext.isSelfAppInBackground() == false
                                || (Build.VERSION.SDK_INT < Build.VERSION_CODES.O || Settings.canDrawOverlays(
        ...
        if (observeSettingsJob.get()?.isCancelled != false) {
            ...
            observeSettingsJob.set(scope.launch {
                settingsRepo.settingsFlow.collect {
                    ...
                    if (lastSettings?.packageListUrl?.toUri()
                                ?.equalsIgnoreSubDomain(it.packageListUrl.toUri()) != true
                    ) {
...
//            .map { LocalDateTime.now() }
    ...
    private fun launchNetworkStateWatcher() {
        if (networkStateJob.get()?.isCancelled != false) {
            ...
            networkStateJob.set(scope.launch {

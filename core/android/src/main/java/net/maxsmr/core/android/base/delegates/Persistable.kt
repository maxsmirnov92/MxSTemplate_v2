...
fun <T> BaseViewModel.persistableStateFlow(initialValue: T): PersistableStateFlow<T> =
    PersistableStateFlow(state, initialValue)
...
/**
 * Используйте, если нужна [StateFlow], переживающая смерть процесса приложения
 */
class PersistableStateFlow<T>(
    ...
    override fun getValue(thisRef: Any, property: KProperty<*>): StateFlow<T> {
        return state.getStateFlow(thisRef.getPersistableKey(property), initialValue)
    }
}

/**
 * Используйте, если нужна [MutableLiveData], переживающая смерть процесса приложения
 */
